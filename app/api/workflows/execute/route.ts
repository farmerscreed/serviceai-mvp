// Workflow Execute API - Task 3.2
// Execute SMS workflows
...
export async function POST(request: NextRequest) {
  try {
    console.log('🔄 Workflow Execute API called')

    // Parse and validate request
    const body = await request.json()
    const validatedData = ExecuteWorkflowRequestSchema.parse(body)
    ...
    const executions = await workflowEngine.executeWorkflow(validatedData.workflowId)

    console.log(`✅ Workflow executed: ${validatedData.workflowId} (${executions.length} steps)`)
